...
if [ ${COUNTBRANCHES} -eq "0" ]; then
  curl -L --request POST "https://${CI_SERVER_HOST}/api/v4/projects/${CI_PROJECT_ID}/merge_requests?id=${CI_PROJECT_ID}&source_branch=${CI_COMMIT_REF_NAME}&target_branch=${TARGET_BRANCH}&title=${TITLE}&remove_source_branch=true&assignee_id=${GITLAB_USER_ID}" --header "Content-Type:application/json" --header "PRIVATE-TOKEN:${PROJECT_ACCESS_TOKEN}"
