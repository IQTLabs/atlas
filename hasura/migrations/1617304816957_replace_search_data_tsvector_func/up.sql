...
$$
    ...
    BEGIN
        ...
        sql = 'SELECT t.data  ->> ''label''::text as label FROM ' ||
              ...
              'd.id = ''' || data_id || ''' AND (to_tsvector(t.data ->> ''label'') @@ to_tsquery(REPLACE(''' || keyword || ''', '' '', '' | '')) ';
        ...
        FOR key IN
            SELECT DISTINCT jsonb_object_keys(t.data -> 'attributes') AS key
                ...
                WHERE id = data_id
            LOOP
                sql := sql || 'OR to_tsvector(t.data -> ''attributes'' ->> regexp_replace(''' || key || ''', ''([(")])'','''',''g'')) @@ to_tsquery(REPLACE(''' || keyword || ''', '' '', '' | ''))  ';
            END LOOP;
        sql := sql || ')';
